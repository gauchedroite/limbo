...
function completely(container: HTMLElement, config: any) {
    ...
    var txtHint = <HTMLInputElement>txtInput.cloneNode();
    txtHint.disabled = true;
    ...
    var createDropDownController = function(elem: HTMLElement) {
        ...
        var p = {
            ...
            refresh: function(token: string, array: string[]) {
                ...
                rows = new Array<HTMLDivElement2>();
                for (var i = 0; i < array.length; i++) {
                    if (array[i].indexOf(token) !== 0) { continue; }
                    var divRow = <HTMLDivElement2>document.createElement('div');
    ...
    dropDownController.onmouseselection = function(text: string) {
    ...
    var rs = {
        ...
        onChange: function(text: string) { rs.repaint() }, // defaults to repainting.
        startFrom: 0,
    ...
    var registerOnTextChangeOldValue: string;
    ...
    **/
    var registerOnTextChange = function(txt: HTMLInputElement, callback: (text: string) => void) {
